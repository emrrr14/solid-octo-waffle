...
pytest -q                              # 67 tests: LP, triggers, valuation, auth, repositories

DEV_MODE=1 python -m scripts.seed_demo # demo user + portfolio + one decision
...
Backend and iOS client are runnable end to end — sign in, stream a live
portfolio, review the proposed allocation, confirm it with Face ID — with 110
tests across the two (67 Python, 43 TypeScript).

Not yet built: Alembic migrations (the dev database is created by `create_all`),
push notifications for rebalance alerts, an intraday history endpoint, and
broker/fund order execution — `confirm` records the decision, it does not route
an order. The Xcode project is generated by `mobile/scripts/bootstrap-ios.sh`
